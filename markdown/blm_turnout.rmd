...
We confirm the statistical presence (second-order spatial effects) of spatial dependence with a global correlation coefficient known as a *Moran’s I* statistic, which tests for “a linear association between a value and a weighted-average of its neighbors” and is often used as a test for spatial autocorrelation [@Ward2008, 23; @Bivand2013, 261]. Table \ref{tab:morans} reports significant, *preliminary*, Moran’s I values for all three key variables: census block group turnout, distance from a protest, and rainfall across the country. Moran’s I values for turnout suggest moderate, positive autocorrelation (0.37), whereas distance reports weak, positive autocorrelation (0.25). Moran’s I for rainfall suggests very high, positive spatial autocorrelation (0.95). These results mean OLS and IV estimates are not reliable and we must correct for spatial dependence in both stages.

\begin{singlespace}
```{r moran, echo = F}
t <- data.frame("Measure" = c("Moran's I", "p-value"),
                "Distance" = c("0.25", "(0.00)"),
                "Turnout" = c("0.38", "(0.00)"),
                "Rainfall" = c("0.95", "(0.00)"))
j <- knitr::kable(t, booktabs = T, caption = "(\\#tab:morans) Moran's I statistics for Distance, Turnout, and Rainfall", linesep = "") %>% 
...
In models 1 and 2 fail to provide statistical support for our hypothesis that block group turnout would be influenced by the distance to a protest. Models 3 and 4, however, demonstrate that there is a causal relationship between protest and turnout at the census tract level: the further a tract was from the nearest protest, the higher its turnout. Models 2 and 4, however, support our expectation that there would be racial heterogeneity in these estimates: while living near a BLM protest apparently had no effect or was demobilizing for non-Black communities, models 2 and 4 indicate that living close to a protest was less demobilizing---or even mobilizing---for Black tracts and block groups.

\begin{singlespace}
\input{"../temp/tract_bg_to.tex"}
\end{singlespace}
...
```{r mef-vf, echo = FALSE, message = F, warning = F, fig.width = 7.5, fig.height = 5, fig.cap = "\\label{fig:mef-t-to}Census Tract Turnout in 2020, by Share Black and Instrumented Distance to a BLM Protest", fig.pos = "h", fig.align = "center"}
p <- readRDS("temp/marg_eff_tract.rds")
p
```
...
The racial resentment scores in the CES are constructed using the following questions, where respondents rank their agreement with the statement on a scale from 1 to 5. While the first two questions are asked of all individuals, only non-white individuals are asked the final three questions.
