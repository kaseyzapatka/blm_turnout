...
lapply(unique(filter(fips_codes, state_code <= 56)$state_code), function(s){
  if(!file.exists(paste0("temp/tracts_", s, ".rds"))){
  zips <- tigris::tracts(state = s, class = "sp")
  ...
  pop <- census_race_ethnicity("tract", 2019, state = s)
  ...
  saveRDS(zip_data, paste0("temp/tracts_", s, ".rds"))
  }})
...
files <- list.files(path = "temp/", pattern = "^tracts_[0-9]", full.names = T)
